...
import { SHAPES } from './shapes.gradient.components';
import {
    normalizeAnimations,
    toAnimationCSS,
} from '../utilities/normalize.gradient.utilities';
import {
    isValidHex,
    stopsFromHex,
} from '../utilities/color.gradient.utilities';
import usePrefersReducedMotion from '../hooks/usePrefersReducedMotion.gradient.hooks';
...
function ensureStyleTag(): HTMLStyleElement | null {
    ...
    if (__ag_styleEl && document.head.contains(__ag_styleEl))
        return __ag_styleEl;
...
const AnimatedGradient: React.FC<AnimatedGradientProps> = ({
    ...
    const shapeLayerStyle: React.CSSProperties | undefined = shapeDef
        ? {
              ...
              maskImage:
                  (shapeDef as any).maskImage ??
                  (shapeDef as any).webkitMaskImage,
              WebkitMaskImage:
                  (shapeDef as any).webkitMaskImage ??
                  (shapeDef as any).maskImage,
              maskSize: (shapeDef as any).maskSize,
              WebkitMaskSize:
                  (shapeDef as any).webkitMaskSize ??
                  (shapeDef as any).maskSize,
              ...
              WebkitMaskRepeat:
                  (shapeDef as any).webkitMaskRepeat ??
                  (shapeDef as any).maskRepeat,
