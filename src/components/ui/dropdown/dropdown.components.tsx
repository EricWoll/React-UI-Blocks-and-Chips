...
function Dropdown({
  ...
  const updateContentSize = useCallback(
    (amount: number) => {
      if (amount <= 0 && amount === contentSize) return;
      setContentSize(amount);
    },
...
function DropdownTrigger({
  ...
  if (asChild) {
    ...
    return cloneElement(child as React.ReactElement<Record<string, unknown>>, {
      ...
      ref: mergeRefs(
        triggerRef,
        (child as React.ReactElement<{ ref?: Ref<HTMLElement> }>).props.ref,
      ),
    });
  }
...
function DropdownContent({
  ...
  }, [isOpen, updatePosition, contentSize]);
  ...
  }, [isOpen, matchContentWidth, triggerRef, contentRef, contentSize]);
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent<HTMLDivElement>) => {
      ...
      switch (e.key) {
        ...
        case "ArrowUp":
          e.preventDefault();
          highlight(items[(currentIndex - 1 + items.length) % items.length]);
          break;
        ...
        case " ":
          e.preventDefault();
          items.find((el) => el.dataset.itemId === highlightedId)?.click();
          break;
  ...
  const resolvedPlacement = Array.isArray(placement) ? placement[0] : placement;
...
function DropdownItem({
  ...
  useLayoutEffect(() => {
    ...
    updateContentSize(contentSize + 1);

    return () => {
      unregisterItem(dropItemId);
      updateContentSize(contentSize - 1);
    };
  ...
  const handleMouseEnter = useCallback(
    (e: MouseEvent<HTMLElement>) => {
      ...
      (onMouseEnter as ((e: MouseEvent<HTMLElement>) => void) | undefined)?.(e);
    },
  ...
  const handleMouseLeave = useCallback(
    (e: MouseEvent<HTMLElement>) => {
      ...
      (onMouseLeave as ((e: MouseEvent<HTMLElement>) => void) | undefined)?.(e);
    },
