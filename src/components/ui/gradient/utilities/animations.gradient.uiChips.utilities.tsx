...
import { uniqueName } from "@/lib/components/uiChips/gradient/components/gradient.uiChips.components";
...
function buildRotateKeyframes(
  ...
  const to = direction === "cw" ? "360deg": "-360deg";
  const css = `
@keyframes ${keyName} {
...
function buildBlobMorphKeyframes(intensity = 1): AnimationDescriptor {
  ...
  const rotation = Math.min(20, 6 * intensity);
  const keyName = uniqueName("ag_blob", String(rotation));
  const css = `
@keyframes ${keyName} {
  0%   { border-radius: 40% 60% 55% 45% / 45% 40% 60% 55%; transform: rotate(0deg) scale(1); }
  25%  { border-radius: 50% 50% 45% 55% / 55% 45% 55% 45%; transform: rotate(${rotation}deg) scale(1.02); }
  50%  { border-radius: 60% 40% 50% 50% / 50% 60% 40% 50%; transform: rotate(0deg) scale(1.04); }
  75%  { border-radius: 45% 55% 60% 40% / 40% 55% 45% 60%; transform: rotate(-${rotation}deg) scale(1.02); }
